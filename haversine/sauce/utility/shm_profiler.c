...
void shm_profiler_dump()
{
    ...
    for (uint32 i = 1; i <= _profiler.timers_count; i++)
    {
        ...
        float64 self_p = (float64)timer->tsc_elapsed_self/(float64)tsc_elapsed * 100.0;
        float64 self_ms = (float64)timer->tsc_elapsed_self/(float64)_profiler.time_counter_frequency * 1000;
        float64 full_p = (float64)timer->tsc_elapsed_full/(float64)tsc_elapsed * 100.0;
        float64 full_ms = (float64)timer->tsc_elapsed_full/(float64)_profiler.time_counter_frequency * 1000;
        float exclusive_p = 
        SHM_TIMER_PRINT("  %s[%u]:\n    self:  %.5f ms, %llu cycles, %.2f%%\n    full: %.5f ms, %llu cycles, %.2f%%\n", 
            timer->key, timer->call_count, self_ms, timer->tsc_elapsed_self, self_p, full_ms, timer->tsc_elapsed_full, full_p);
    }
